...
/** Immersive voice capture — always lands on the Hinglish demo if SR fails */
export const VoicePage: React.FC = () => {
  const { navigate, language, updateDossier } = useApp();
  const [phase, setPhase] = useState<Phase>('listening');
  const [transcript, setTranscript] = useState('');
  const [seconds, setSeconds] = useState(0);
  const [status, setStatus] = useState('Listening… speak freely');
  ...
  useEffect(() => {
    ...
    setSeconds(0);
    setTranscript('');
    setStatus('Listening… tell SANAD what happened');
    timerRef.current = window.setInterval(() => setSeconds((s) => s + 1), 1000);

    if (!isSpeechRecognitionSupported()) {
      demoRef.current = window.setTimeout(() => {
        finishWith(DEMO_HINGLISH_TRANSCRIPT, 'Demo transcript ready — edit if you need to');
      }, 2800);
      return cleanup;
    }
    ...
    try {
      const recognizer = createSpeechRecognizer(
        ...
        () => finishWith(DEMO_HINGLISH_TRANSCRIPT, 'Mic unavailable — demo statement loaded'),
        () => {
          setTranscript((prev) => {
