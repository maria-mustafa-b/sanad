...
import { ShieldCheck, Mic, FileSearch, CheckCircle, Smartphone } from "lucide-react";
...
export function SanadHero() {
  ...
  return (
    ...
    >
      ...
      <div
        ref={imageRef}
        className="absolute inset-0 w-full h-[115%] -top-[5%] z-0"
      >
        <Image
      ...
      <div
        ref={contentRef}
        className="relative z-10 w-full max-w-5xl mx-auto px-4 flex flex-col items-center md:items-start text-center md:text-left mt-16 md:mt-0"
      >
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto mb-16">
          <Link
            href="/chat"
            className="w-full sm:w-auto px-8 py-4 bg-emerald-600 text-white rounded-lg font-bold text-lg hover:bg-emerald-500 transition shadow-[0_0_20px_rgba(5,150,105,0.4)] flex items-center justify-center gap-2"
          >
            Get Started
          </Link>
          <Link
            href="#how-it-works"
            className="w-full sm:w-auto px-8 py-4 bg-white/10 text-white backdrop-blur-md border border-white/20 rounded-lg font-bold text-lg hover:bg-white/20 transition flex items-center justify-center gap-2"
          >
            Explore SANAD
          </Link>
        </div>
